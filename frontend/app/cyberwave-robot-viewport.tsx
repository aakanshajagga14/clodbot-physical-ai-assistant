...
export default function CyberwaveRobotViewport({ action, moving, viewMode = "reset", compact = false }: Props) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        scene.add(new THREE.HemisphereLight(0xffffff, 0x748078, compact ? 2.1 : 2.35));
        const key = new THREE.DirectionalLight(0xffffff, 3.1);
        key.position.set(1.8, 2.4, 1.6);
        key.castShadow = !compact;
        key.shadow.mapSize.set(1024, 1024);
        scene.add(key);
        const rim = new THREE.DirectionalLight(0xc7e7db, 1.25);
